...
export default function Header({ onMenuClick }) {
  ...
  return (
    <header className="sticky top-0 z-20 h-16 flex items-center gap-4 px-4 sm:px-6 bg-white/90 dark:bg-primary-800/90 backdrop-blur border-b border-slate-100 dark:border-primary-700">
      ...
      <div className="relative">
        ...
        {menuOpen && (
          <div className="absolute right-0 mt-2 w-48 card p-1.5 text-sm">
            ...
            <button
              onClick={logout}
              className="w-full text-left px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-primary-700 text-accent-600"
            >
              Log out
            </button>
          </div>
